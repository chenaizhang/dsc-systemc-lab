...
| T2 | UHDM 结构重建 | `inputs/private/rtl/surelog.f` | 261 个实例层次已恢复；新端口/连接结构指纹待完成 |
| T3 | CIRCT 分阶段定位 | T2 RTL/UHDM、CIRCT | 两个最小复现已完成；conversion/emission patch 待完成 |
...
| T6 | 差分与混合替换 | T4、T5、Verilator --sc | 首错定位到 format/stream；只缺正式 last/flush 实现或两行正确波形 |
