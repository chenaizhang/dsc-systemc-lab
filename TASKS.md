...
| T3 | CIRCT 分阶段定位 | T2 RTL/UHDM、CIRCT | HW-only 已生成并运行 261 实例；Comb/Seq 首阻塞为 `hw.bitcast`，行为线继续中 |
| T4 | 分层 Function SystemC | T2 结构合同、顶层 function、逐模块 SV | UHDM 同构骨架和逐层计划已通过；仅顶层 function 已验证，深度 1～5 待实现 |
...
- Function 线：顶层已验证；后续按 7、24、45、92、92 个实例逐层细化。
- 行为线：Comb/Seq SSA 已提取，完整 SystemC lowering 首先停在 aggregate `hw.bitcast`。
